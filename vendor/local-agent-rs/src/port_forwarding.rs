...
async fn request_port_forwarding(
    ...
    let mut timeout_duration = std::time::Duration::from_secs(timeout_in_seconds);
    ...
    for attempt in 0..max_retries + 1 {
        ...
        // the send.
        tokio::time::timeout(timeout_duration, transport.send(&request_u8.clone())).await??;
        ...
        match tokio::time::timeout(timeout_duration, transport.recv()).await {
            Ok(Ok(response_u8)) => {
                let response: Response = serializer.deserialize(&response_u8)?;
...
/// If the port forwarding fails, an error is returned.
///
pub async fn request_tcp_port_forwarding(timeout_in_seconds: u64, max_retries: u32) -> Result<u16> {
...
mod tests {
    ...
    async fn test_request_port_forwarding() -> Result<()> {
        ...
        assert_eq!(
            request_port_forwarding(&mock_transport, NatPmpRequestOp::Tcp, 2, 3,).await?,
            123_u16
        );
    ...
    async fn test_failed_port_forwarding() -> Result<()> {
        ...
        match request_port_forwarding(&mock_transport, NatPmpRequestOp::Tcp, 2, 3).await {
            Err(Error::PortForwarding(e)) => {
                assert_eq!(e, "Failed to request port forwarding, response code: 4");
            }
            _ => panic!("expected PortForwarding error"),
        }
