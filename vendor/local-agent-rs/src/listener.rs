use crate::{
    request_tcp_port_forwarding, AgentConnection, AgentConnector, AgentFeatures, ConnectParams,
...
impl Listener {
    ...
    pub async fn listen<C>(&self, callback: C) -> Result<()>
    where
            ...
            match new_status_future {
                Ok(new_status) => {
                    match new_status {
                        ...
                        Err(Error::GetStatusError(error)) => {
                            callback(Err(Error::GetStatusError(error)))?
                        }
                        Err(error) => {
    ...
    async fn request_port_forwarding_lease(&self, status: &mut StatusMessage) -> Result<bool> {
        ...
        {
            let port = request_tcp_port_forwarding(
                DEFAULT_TIMEOUT_IN_SECONDS,
                MAX_PORT_FORWARDING_RETRIES,
            )
            .await?;
    ...
    /// Requests a pushed status update including feature statistics.
    pub async fn request_status(
        &self,
        features_statistics: Option<bool>,
        timeout_in_seconds: u64,
    ) -> Result<()> {
        self.connection
            .request_status(timeout_in_seconds, features_statistics)
            .await
    }
}
