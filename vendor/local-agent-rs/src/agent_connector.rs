...
// -----------------------------------------------------------------------------
use crate::{
    transport_playback::TransportPlayback, transport_stream::TransportStream, AgentConnection,
    Error, Result,
};
// -----------------------------------------------------------------------------
...
fn build_root_cert_store() -> Result<RootCertStore> {
    let mut cursor = std::io::Cursor::new(PROTON_VPN_ROOT_FOLLOWED_BY_INTERMEDIATE_LEGACY_CA);
...
fn ensure_certificates_are_current(
    ...
    for cert_d in certificates {
        let (_, cert) = x509_parser::parse_x509_certificate(cert_d)
            .map_err(|_| Error::UnableToParseCertificate)?;
...
impl AgentConnector {
    ...
    ///
    pub async fn connect(params: ConnectParams) -> Result<AgentConnection> {
        ...
        let dnsname = rustls_pki_types::ServerName::try_from(params.domain.clone())?;

        let timeout_duration = std::time::Duration::from_secs(params.timeout_in_seconds);

        let tcp_stream =
            tokio::time::timeout(timeout_duration, TcpStream::connect(SERVER_ADDR)).await??;
        ...
        let connection =
            tokio::time::timeout(timeout_duration, connector.connect(dnsname, tcp_stream))
                .await??;
...
mod tests {
    ...
    fn test_proton_vpn_root_and_intermediate_legacy_certificates_are_not_close_to_expiration() {
        ...
        let mut reader = std::io::Cursor::new(PROTON_VPN_ROOT_FOLLOWED_BY_INTERMEDIATE_LEGACY_CA);
        ...
        for cert_der in rustls_pemfile::certs(&mut reader) {
            ...
            let cert =
                x509_parser::parse_x509_certificate(&(cert_i)).expect("Certificate does not parse");
    ...
    fn test_valid_certificate_with_current_time() {
        ...
        assert!(
            result.is_ok(),
            "Embedded certificates should be currently valid"
        );
    }
    ...
    fn test_expired_certificate_error() {
        ...
        assert!(
            matches!(result, Err(Error::ExpiredCertificate(_))),
    ...
    fn test_not_yet_valid_certificate_error() {
        ...
        assert!(
            matches!(result, Err(Error::NotYetValidCertificate(_))),
